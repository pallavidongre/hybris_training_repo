    ...
    <c:otherwise>
                                    ...
                                    <a href="${fn:escapeXml(productStyleUrl)}" class="colorVariant"
                                       name="${fn:escapeXml(variantStyle.url)}">
                                        <c:if test="${not empty imageData}">
                                            <img src="${fn:escapeXml(imageData.url)}" title="${styleValueHtml}"
                                                 alt="${styleValueHtml}"/>
                                        </c:if>
        ...
        <c:if test="${not empty variantOptions}">
            <div class="variant-section">
                <div class="variant-selector">
                    <div class="variant-name">
                        <label for="Flag">Flag: <span>${product.flags}</span></label>
                    </div>
                    <div class="variant-name">
                    ...
                    <div>
                        <button onclick="loadJsonData()">Manufacture Details</button>
                        <div id="manufactureJsonData">
                        </div>
                        <br>
                        <script>
                            function loadJsonData() {
                                ...
                                xhttp.onreadystatechange = function () {
                                    if (this.readyState == 4 && this.status==200) {
                                        document.getElementById("manufactureJsonData").innerHTML = this.responseText;
                                    }
                                };
                                xhttp.open('GET', '${pageContext.request.contextPath}/p/company/Apple', true)
                                xhttp.send()
                            }
                        </script>
                    </div>

                    <div class="variant-name">F
                        <label for="customSEO">${product.customSEO}</label>
                    </div>
                    <br>
