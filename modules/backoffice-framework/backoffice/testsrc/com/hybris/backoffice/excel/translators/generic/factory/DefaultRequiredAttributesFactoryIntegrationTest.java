...
public class DefaultRequiredAttributesFactoryIntegrationTest extends ServicelayerTest
{
	...
	{
		// given
		final AttributeDescriptorModel localizedAtomic = typeService.getAttributeDescriptor(ProductModel._TYPECODE,
				ProductModel.NAME);

		// when
		final RequiredAttribute requiredAttribute = requiredAttributesFactory.create(localizedAtomic);

		// then
		assertThat(requiredAttribute.getEnclosingType()).isEqualTo(ProductModel._TYPECODE);
		assertThat(requiredAttribute.getQualifier()).isEqualTo(ProductModel.NAME);
		assertThat(requiredAttribute.getChildren()).isEmpty();
	}
}
