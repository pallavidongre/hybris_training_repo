...
public class SyncTrackerControllerTest extends AbstractWidgetUnitTest<SyncTrackerController>
{
	...
	{
		//given
		final SyncTaskExecutionInfo executionInfo = mockExecutionInfo("cronJobCode", 1, 2);
		//when
	...
	public void shouldUpdateTrackingMapWhenUpdatedProductIsInTrackedCatalogVersionSynchronization()
	{
		...
		controller.getTrackingMap().put("cronJobCode", trackedCatalogVersionsPks);
		...
		// then
		// catalog version is being tracked
		assertThat(controller.getTrackingMap().get("cronJobCode")).contains(String.valueOf(trackedCatalogVersionPk));
		// updated product is also being tracked
		assertThat(controller.getTrackingMap().get("cronJobCode")).contains(String.valueOf(updatedProductPk));
	}
